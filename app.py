...
.vtab-active button {
    ...
    box-shadow: 4px 4px 0 rgba(255,107,53,0.3) !important;
    transform: translateX(4px) !important;
}
</style>
...
_nav_col, _content_col = st.columns([1, 4])

with _nav_col:
    st.markdown("<br>", unsafe_allow_html=True)
    for _tab_label in _TAB_LABELS:
        _is_active = st.session_state.active_tab == _tab_label
        _css_class = "vtab-active" if _is_active else "vtab-btn"
        st.markdown(f'<div class="{_css_class}">', unsafe_allow_html=True)
        if st.button(_tab_label, key=f"nav_{_tab_label}", use_container_width=True):
            st.session_state.active_tab = _tab_label
            st.rerun()
